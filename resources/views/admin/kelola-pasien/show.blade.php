...
@section('content')
                ...
                <div class="card border-0 shadow-sm rounded overflow-hidden">
                    <div class="card-body p-0">
                        <div class="d-flex justify-content-between align-items-center py-2 px-3 bg-primary-subtle">
                            ...
                            <div>
                                <a href="{{ route('admin.pasien.index') }}" class="btn btn-secondary me-2 px-3 shadow-sm">
        ...
        <div class="row g-3 mb-3">
            <div class="col-lg-6">
                <div class="p-3 bg-white shadow-sm rounded-3 border-0 h-100 small">
                    <h6 class="fw-bold mb-3 text-dark border-bottom pb-2">Profil Pasien</h6>
                    <div class="d-flex mb-2 align-items-center">
                        <span class="text-muted flex-shrink-0" style="width: 120px;">Nama Lengkap</span>
                        <span class="fw-bold text-dark text-truncate">: {{ $pasien->name }}</span>
                    </div>
                    <div class="d-flex mb-2 align-items-center">
                        <span class="text-muted flex-shrink-0" style="width: 120px;">Email</span>
                        <span class="fw-bold text-dark text-truncate">: {{ $pasien->email }}</span>
                    </div>
                    <div class="d-flex align-items-center">
                        <span class="text-muted flex-shrink-0" style="width: 120px;">Nomor HP</span>
                        <span class="fw-bold text-dark">: {{ $pasien->no_hp ?? '-' }}</span>
                    </div>
                </div>
            </div>
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="text-center text-muted py-3">
                                    Belum ada data kendaraan terdaftar untuk pasien ini.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content rounded-4 border-0 shadow">
                <div class="modal-header border-0">
                    <h5 class="fw-bold" id="modalTitle">Tambah Kendaraan Pasien</h5>
                    <button class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>

                <form id="formKendaraan" action="{{ route('admin.kendaraan.store') }}" method="POST">
                    @csrf
                    <input type="hidden" name="user_id" id="user_id" value="{{ $pasien->id }}">
    ...
    <script>
        ...
        function openModalTambahKendaraan() {
            ...
            document.getElementById('user_id').value = "{{ $pasien->id }}";
