...
@section('title', 'Dashboard Pasien | Smart Parking')
    ...
    <div class="row g-2 text-center">
        @php
            $stats = [
                ...
                ['label' => 'Total Pengeluaran', 'val' => 'Rp ' . number_format($totalPengeluaran, 0, ',', '.'), 'class' => 'text-primary', 'full' => true]
            ];
        @endphp
        ...
        <div class="modal-content border-0 shadow" style="border-radius: 20px;">
            <form id="formParkir">
                ...
                <div id="modalBodyContainer" class="modal-body p-4">
                    ...
                    <div class="mt-4">
                        <label class="form-label small fw-semibold">Tarif Berlaku</label>
                        <div class="input-group">
                            <span class="input-group-text border-0 bg-light text-muted">Rp</span>
                            <input type="text" id="tarifDisplay" class="form-control border-0 bg-light fw-bold text-primary" readonly value="Tarif Sesuai Durasi/Jenis">
                        </div>
                    </div>
                </div>
...
<script>
    function updateAllTimers() {
        document.querySelectorAll('.parkir-card').forEach(card => {
            ...
            if (costEl) {
                // Kalkulasi tarif dapat disesuaikan dengan daftar $tarifs jika diperlukan secara dinamis
                costEl.innerText = '0'; 
            }
        });
    }
    ...
    function submitData() {
        ...
        fetch("{{ route('pasien.parkir.store') }}", {
            method: "POST",
            body: formData,
            headers: {
        ...
        .then(async response => {
            ...
            Swal.fire({
                icon: 'success',
                title: 'Berhasil',
                text: 'Silakan menuju halaman QR / Akses Masuk.',
                timer: 2000,
