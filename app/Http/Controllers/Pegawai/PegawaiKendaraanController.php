...
class PegawaiKendaraanController extends Controller
{
    public function index()
    {
        ...
        return view('pegawai.kendaraan.index', compact('kendaraan', 'user'));
    }
    ...
    public function detail($id)
    {
        ...
        // Bagi pegawai total pembayaran kumulatif selalu 0
        $totalSemua = 0;
        ...
        $parkirAktif = ParkirTransaksi::where('kendaraan_id', $kendaraan->id)
            ...
            ->map(function ($t) {
                ...
                $t->estimasi_bayar = 0; // Dikunci ke 0 rupiah karena gratis

                return $t;
            });

        return view('pegawai.kendaraan.detail', compact(
            'user',
            'kendaraan',
            'transaksiRiwayat',
            'parkirAktif',
    ...
    public function detailHistory($id)
    {
        ...
        if ($transaksi->status === 'aktif') {
            ...
            $transaksi->estimasi_biaya = 0; // Pegawai tidak dikenakan biaya estimasi
        }

        return view('pegawai.kendaraan.detailHistory', compact('transaksi'));
    }
    ...
    public function store(Request $request)
    {
        try {
            ...
            return redirect()->back()->with('success', 'Kendaraan pegawai berhasil ditambahkan');
            ...
            Log::error('Pegawai gagal tambah kendaraan', [
                'user_id' => Auth::id(),
                'error' => $e->getMessage()
            ]);
        ...
        try {
            ...
            if ($request->ajax()) {
                return response()->json([
                    'success' => true,
                    'message' => 'Sesi parkir pegawai siap dimulai',
                    'redirect' => route('pegawai.parkir.scan', $kendaraan->id) // Penyesuaian nama route scanner pegawai
                ]);
            }

            return redirect()->route('pegawai.parkir.scan', $kendaraan->id);
        } catch (\Illuminate\Validation\ValidationException $e) {
            if ($request->ajax()) {
                return response()->json([
            ...
            Log::error('Pegawai gagal proses parkir', [
                'user_id' => Auth::id(),
                'error' => $e->getMessage()
            ]);
    ...
    public function getStatusApi($id)
    {
        ...
        return response()->json([
            ...
            'total_bayar_formatted' => "Free" // Di-hardcode text "Free" untuk API Real-time view detail pegawai
        ]);
    }
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            return redirect()->back()->with('success', 'Data kendaraan pegawai berhasil diperbarui');
        } catch (\Illuminate\Validation\ValidationException $e) {
            return redirect()->back()
                ->withErrors($e->validator)
                ->withInput()
            ...
            Log::error('Pegawai gagal update kendaraan', [
                'user_id' => Auth::id(),
                'kendaraan_id' => $id,
                'error' => $e->getMessage()
            ]);
