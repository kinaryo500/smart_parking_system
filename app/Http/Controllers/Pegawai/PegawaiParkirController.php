...
class PegawaiParkirController extends Controller
{
    public function scanIndex($kendaraanId)
    {
        ...
        return view('pegawai.parkir.scan', compact('kendaraan'));
    }
    ...
    public function detail($id)
    {
        ...
        if ($transaksi->status === 'aktif') {
            ...
            $transaksi->estimasi_biaya = 0; // Pegawai selalu Free
        }

        return view('pegawai.parkir.detail', compact('transaksi'));
    }
    ...
    public function scanStore(Request $request)
    {
        ...
        try {
            ...
            $transaksi = ParkirTransaksi::create([
                ...
                'tarif_per_jam'   => 0, // Set tarif 0 untuk pegawai
                'status'          => 'aktif'
            ]);
            ...
            return response()->json([
                ...
                'redirect' => route('pegawai.dashboard.parkir.detail', $transaksi->id)
            ]);
        } catch (\Exception $e) {
            DB::rollBack();
            Log::error("Pegawai Scan MQTT Error: " . $e->getMessage());
